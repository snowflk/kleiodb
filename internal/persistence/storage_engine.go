...
type StorageEngine interface {
	...
	// StorageEngine does not keep track of sequence number, so it must rely on the higher-level to pass the latest sequence number
	// It's the higher-level module's responsibility to maintain sequence number's uniqueness and correctness
	//
	...
	Append(streamName string, startingFrom uint64, data []Bytearray) ([]uint64, error)
	...
	AddToIndex(indexName string, serials []uint64) error
	...
	GetFromStream(streamName string, offset, limit uint64) ([]RawEvent, error)

	// Get events from a given stream but only indexed ones
	// If the stream or the index does not exists, an error will be returned
	// Offset and Limit are analogous to SQL
	GetFromIndex(indexName string, offset, limit uint64) ([]RawEvent, error)

	// Get events from the global stream
	// Offset and Limit are analogous to SQL
	GetFromGlobalStream(offset, limit uint64) ([]RawEvent, error)
